...
// ── Pandemonium weapons (rareté pandemonium) ──
// Données utilisées par les calculs de stats sur les fiches/inventaire.
const PANDEMONIUM_ITEMS={
...
const PANDEMONIUM_SOLO_BONUS_VAL=310;
const PANDEMONIUM_PARTY_BONUS_VAL=540;
const SLOT_LIMITS={
...
function renderStatsSummary(){
  ...
  const charRace=(CHAR&&(CHAR.race||CHAR.race_category))||'';

  // 1) Standard equipment bonuses (skip signature + equalizer + pandemonium)
  equipped.forEach(id=>{
    const it=ALL_ITEMS_DATA[id]||{};
  ...
  // 2c) Pandemonium weapons (party-conditional). On regarde la PARTY_DATA
  //     pré-chargée pour détecter un autre porteur d'arme Pandemonium.
  if(equipped.some(id=>PANDEMONIUM_ITEMS&&PANDEMONIUM_ITEMS[id])){
    let synergy=false;
    try{
      ...
      for(const m of members){
        ...
        // PARTY_DATA peut embarquer les inventaires des membres dans `equipped` directement
        const mEq=(m.equipped_assets||m.equipped||[]);
        if(mEq.some(eid=>PANDEMONIUM_ITEMS[eid])){synergy=true;break;}
      }
    }catch(_){}
    const pdmBonuses=calculatePandemoniumBonuses(equipped,synergy);
    Object.entries(pdmBonuses).forEach(([s,v])=>{totals[s]=(totals[s]||0)+v;});
  }

  // 3) Set bonuses (highest threshold only) — passe la race pour race_bonus
  const setResult=calculateSetBonuses(equipped,charRace);
...
function renderSetsPanel(){
  const equipped=new Set(INV_DATA.equipped_assets||[]);
  const activeSets=[];
  const rc={common:'#6b7280',uncommon:'#60a5fa',rare:'#a78bfa',epic:'#8B5CF6',
            racial:'#14b8a6',
            legendary:'#fbbf24',mythic:'#f97316',unique:'#ffd60a',
            pandemonium:'#9d00ff',
  ...
  el.innerHTML=activeSets.map(({setId,setDef,count,thresholds,activeThresh,equippedCount})=>{
    ...
    const bonusHtml=activeThresh.map(t=>{
      ...
      // Affichage race_bonus : actif uniquement si la race correspond
      if(b.race_bonus){
        const rbRace=(b.race_bonus.race||'').toString().trim();
        const isActive=rbRace.toLowerCase()===_charRace;
        const rbParts=[];
        if(b.race_bonus.stats)Object.entries(b.race_bonus.stats).forEach(([s,v])=>rbParts.push(`+${v} ${SI[s]||s}`));
        if(b.race_bonus.stats_all)rbParts.push(`+${b.race_bonus.stats_all} tous stats`);
        if(rbParts.length){
          const tag=isActive?`<span style="color:#14b8a6">(${rbRace} ✓)</span>`:`<span style="opacity:.5">(${rbRace} only)</span>`;
          parts.push(`${tag} ${rbParts.join(' · ')}`);
        }
      }
